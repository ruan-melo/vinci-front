...
import { AnnotationIcon, BellIcon, ChevronDownIcon, HeartIcon, UserAddIcon } from '@heroicons/react/solid';
...
export function Notifications({notifications}: NotificationsProps) {
    ...
    return (
      <div className="px-4 ml-auto md:ml-0 mr-0">
                ...
                <Popover.Panel className="fixed sm:absolute right-1/2 translate-x-1/2 md:right-1/2 z-10 mt-3 w-screen max-w-sm transform px-2 lg:px-4 sm:px-0 lg:max-w-md">
                  <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                                ...
                                <>
                                    <div className=" relative mr-2 flex align-center items-center  ">
                                        <div className='relative w-8 h-8 rounded-full overflow-hidden'>
                                            <Image src={item.user.avatar} width={'100%'} height={'100%'} alt={`Foto de perfil ${item.user.name}`}/>
                                        </div>
                                        
                                        <span className='ml-2 text-gray-400 text-lg'>
                                            {item.type === 'follow' &&  <UserAddIcon className= 'w-4 h-4' />}
                                            {item.type === 'like' &&  <HeartIcon className= 'w-4 h-4' />}
                                            {item.type === 'comment' &&  <AnnotationIcon className= 'w-4 h-4' />}
                                        </span>
                                    </div>
                                    {item.type === 'follow' &&  <span className='text-sm'>{item.user.name} followed you</span> }
                                    {item.type === 'like' &&  <span className='text-sm'>{item.user.name} liked your post</span> }
                                    {item.type === 'comment' &&  <span className='text-sm'>{item.user.name} commented on your post</span> }
                    ...
                    <div className="bg-gray-50 p-2 px-4 inline-flex justify-center items-end w-full ">
                        <span className=' text-sm text-green-700 hover:text-green-900 transition-all duration-200 cursor-pointer'>Clear all</span>
                      {/* <a
                        href="##"
